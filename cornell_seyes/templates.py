...
def build_actief_leren_pdf(output_path: str, cfg: LayoutConfig = LayoutConfig()) -> str:
    ...
    back_bottom = Table(
        [
            [Paragraph("<font color=grey><b>Reflectie / vragen</b></font> <font size=8 color=grey>(Wat snap ik nog niet? Waar heb ik extra uitleg over nodig?)</font>", ParagraphStyle("sl", fontSize=10))],
...
def build_wiskunde_pdf(output_path: str, cfg: LayoutConfig = LayoutConfig()) -> str:
    ...
    summary_label = Paragraph(
        "<font color=grey><b>Samenvatting</b></font>",
        ParagraphStyle("sl", fontSize=10)
    )
...
def build_millimeter_pdf(output_path: str, cfg: LayoutConfig = LayoutConfig()) -> str:
    page_w, page_h = A4
    usable_w = page_w - cfg.left_margin_pt - cfg.right_margin_pt
    usable_h = page_h - cfg.top_margin_pt - cfg.bottom_margin_pt
    ...
    cornell = Table(
        [
            [left_header, right_header],
            [CarreBlock(left_col, notes_h, 1 * mm), CarreBlock(right_col, notes_h, 1 * mm)],
        ],
        colWidths=[left_col, right_col],
        rowHeights=[cfg.cornell_header_h, notes_h],
    )
    ...
    bottom = Table(
        [
            [summary_label],
            [CarreBlock(usable_w, cfg.write_h, 1 * mm)],
        ],
        colWidths=[usable_w],
        rowHeights=[cfg.label_h, cfg.write_h],
    )
    ...
    back_cornell = Table(
        [
            [right_header],
            [CarreBlock(usable_w, back_notes_h, 1 * mm)],
        ],
        colWidths=[usable_w],
        rowHeights=[cfg.cornell_header_h, back_notes_h],
    )
